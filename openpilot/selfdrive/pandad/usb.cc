...
#define USB_PID_BLACK_PANDA 0xddcc
#define USB_PID_UNO 0xddee
...
static bool is_panda_usb_id(uint16_t idVendor, uint16_t idProduct) {
  return idVendor == USB_VID && (idProduct == USB_PID_BLACK_PANDA || idProduct == USB_PID_UNO);
}
...
PandaUsbHandle::PandaUsbHandle(std::string serial) : PandaCommsHandle(serial) {
  ...
  for (size_t i = 0; i < num_devices; ++i) {
    ...
    if (is_panda_usb_id(desc.idVendor, desc.idProduct)) {
