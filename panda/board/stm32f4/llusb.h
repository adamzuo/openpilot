...
void usb_init(void) {
  ...
  // Full OTG peripheral reset: clears any state left behind by a previous USB
  // session (e.g. the ST ROM bootloader after DFU). Without this the AHBIDL
  // wait below can hang when booting after a bootloader session.
